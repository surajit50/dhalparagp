...
export default async function StaffComplaintDetailPage({
  ...
  const electricAgencies = await db.agencyDetails.findMany({
    ...
    <div className="space-y-8 p-4 sm:p-6 lg:p-8 bg-gradient-to-br from-background to-muted/30 min-h-screen">
      ...
      <div className="rounded-xl border border-border/50 bg-card p-6 space-y-4">
        ...
        <div className="grid grid-cols-2 gap-4 text-sm">
          ...
          <DetailRow label="Assigned Staff" value={complaint.assignedStaff?.name ?? "—"} />
          <DetailRow label="Assigned Agency" value={complaint.assignedAgency?.name ?? "—"} />
