...
interface EditWorkDetailsClientProps {
  initialWorkDetails: any[]
  schemeNames: string[]
}

export function EditWorkDetailsClient({ initialWorkDetails, schemeNames }: EditWorkDetailsClientProps) {
      ...
      <Card className="bg-white border-none shadow-md">
        <CardHeader className="pb-4 border-b">
          <div className="flex items-center space-x-4">
            ...
            <div>
              <CardTitle className="text-lg font-bold text-gray-800">Filter Works (Edit Mode)</CardTitle>
              <CardDescription>Refine your search by scheme or NIT number to edit payment details</CardDescription>
            </div>
          </div>
        </CardHeader>
            ...
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-gray-500">
                  {(selectedScheme === "all" && selectedNit === "all") ? "Total Works with Payments" : "Filtered Works"}
                </p>
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <CardDescription className="mt-1">
                Manage and update payment details for ongoing projects
              </CardDescription>
            </div>
                  ...
                  filteredWorkDetails.map((work, index) => (
                    <TableRow key={work.id} className="hover:bg-gray-50/50">
                      ...
                      <TableCell className="text-center">
                        {(() => {
                          const payments = work.paymentDetails || []
                          const latestPayment = payments[payments.length - 1]

                          const initialValues = latestPayment
                            ? {
                                grossBillAmount: latestPayment.grossBillAmount ?? 0,
                                mbrefno: latestPayment.mbrefno ?? "",
                                billType: latestPayment.billType ?? undefined,
                                billPaymentDate: latestPayment.billPaymentDate
                                  ? new Date(latestPayment.billPaymentDate)
                                  : new Date(),
                                workcompletaitiondate: latestPayment.workcompletaitiondate
                                  ? new Date(latestPayment.workcompletaitiondate)
                                  : undefined,
                                eGramVoucher: latestPayment.eGramVoucher ?? "",
                                eGramVoucherDate: latestPayment.eGramVoucherDate
                                  ? new Date(latestPayment.eGramVoucherDate)
                                  : new Date(),
                                gpmsVoucherNumber: latestPayment.gpmsVoucherNumber ?? "",
                                gpmsVoucherDate: latestPayment.gpmsVoucherDate
                                  ? new Date(latestPayment.gpmsVoucherDate)
                                  : new Date(),
                                netAmount: latestPayment.netAmt ?? 0,
                                lessIncomeTax: latestPayment.lessIncomeTax?.incomeTaaxAmount ?? 0,
                                lessLabourWelfareCess:
                                  latestPayment.lessLabourWelfareCess?.labourWelfarecessAmt ?? 0,
                                lessTdsCgst: latestPayment.lessTdsCgst?.tdscgstAmt ?? 0,
                                lessTdsSgst: latestPayment.lessTdsSgst?.tdsSgstAmt ?? 0,
                                securityDeposit: latestPayment.securityDeposit?.securityDepositAmt ?? 0,
                              }
                            : undefined

                          return (
                            <Dialog
                              open={openDialog && selectedWorkId === work.id}
                                ...
                                <div className="py-4">
                                  {selectedWorkId && latestPayment && (
                                    <AddPaymentDetailsForm
                                      ...
                                      paymentDetailsId={latestPayment.id}
                  ...
                  <TableRow>
                    <TableCell colSpan={7} className="p-8 text-center text-gray-500">
                      <div className="flex flex-col items-center justify-center py-8">
                        ...
                        <p className="text-lg">
                          {(selectedScheme === "all" && selectedNit === "all")
                            ? "No works with payment details found"
                            : "No works found matching the selected filters"}
                        </p>
                        <p className="text-sm text-gray-400 mt-1">
                          {(selectedScheme === "all" && selectedNit === "all")
                            ? "Once payments are recorded, they will appear here for editing"
