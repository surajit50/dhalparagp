...
import { Loader2, Save, Trash2, RotateCcw, HelpCircle } from "lucide-react";
...
export default function Form36Client() {
  ...
  const [selectedYear, setSelectedYear] = useState(years[2]); // Default 2025-2026
  
  // Compute preceding, current, next year strings
  const startYear = parseInt(selectedYear.split("-")[0]);
  const precedingYearLabel = `${startYear - 1}-${(startYear).toString().slice(2)}`;
  const currentYearLabel = `${startYear}-${(startYear + 1).toString().slice(2)}`;
  ...
  const fetchBudget = async () => {
    ...
    // Set opening balance if it was saved
  ...
  const autoFillEstimates = async () => {
    ...
    if (result.success && result.data) {
      ...
      const updatedRows = rows.map((row) => {
        if ((row as any).isHeader) return row;
  ...
  const handleObChange = (field: keyof typeof openingBalance, value: string) => {
    setOpeningBalance({ ...openingBalance, [field]: Number(value) });
  };

  const saveRow = async (index: number) => {
    const row = rows[index];
    if ((row as any).isHeader) return;
    ...
    if (res.success) {
      ...
      toast({ title: "Error", description: `Failed to save ${row.fundName}.`, variant: "destructive" });
    }
  };
  ...
  const saveAll = async () => {
    ...
    // Save all data rows
    const promises = rows.filter(r => !(r as any).isHeader).map(row => 
      saveForm36Budget({ financialYear: selectedYear, ...row })
    );

    // Save opening balances as pseudo-funds
    promises.push(saveForm36Budget({
      financialYear: selectedYear, fundName: "Opening Balance Preceding",
      precedingYearActual: openingBalance.preceding, currentYearEstimate: 0, nextYearEstimate: 0
    }));
    promises.push(saveForm36Budget({
      financialYear: selectedYear, fundName: "Opening Balance Current",
      precedingYearActual: 0, currentYearEstimate: openingBalance.current, nextYearEstimate: 0
    }));
    promises.push(saveForm36Budget({
      financialYear: selectedYear, fundName: "Opening Balance Next",
      precedingYearActual: 0, currentYearEstimate: 0, nextYearEstimate: openingBalance.next
    }));
          ...
          <Table className="border-collapse">
            <TableHeader className="bg-gray-100 sticky top-0 z-10 shadow-sm">
              ...
              <TableRow className="border-b-2 border-black bg-gray-50">
                ...
                <TableHead className="font-bold text-black border-r text-center align-middle w-[15%]">
                  Annual receipts of the<br/>preceding year<br/>{precedingYearLabel}
                </TableHead>
                <TableHead className="font-bold text-black border-r text-center align-middle w-[15%]">
                  Budget estimate of the<br/>current year<br/>{currentYearLabel}
                </TableHead>
                <TableHead className="font-bold text-black border-r text-center align-middle w-[15%]">
                  Budget estimate for the<br/>next year<br/>{nextYearLabel}
                </TableHead>
                <TableHead className="font-bold text-black border-r text-center align-middle w-[15%]">
                  Remarks
                </TableHead>
            ...
            <TableBody>
              ...
              <TableRow className="bg-gray-50 border-t-2 border-black font-bold">
                <TableCell className="p-2 border-r border-gray-300 text-center">
                  Total -
                </TableCell>
                <TableCell className="p-2 border-r border-gray-300 text-right tabular-nums text-sm">
                  ₹ {totalPreceding.toLocaleString("en-IN", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                </TableCell>
                <TableCell className="p-2 border-r border-gray-300 text-right tabular-nums text-sm">
                  ₹ {totalCurrent.toLocaleString("en-IN", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                </TableCell>
                <TableCell className="p-2 border-r border-gray-300 text-right tabular-nums text-sm">
                  ₹ {totalNext.toLocaleString("en-IN", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                </TableCell>
              ...
              <TableRow className="bg-red-50 text-red-600 font-bold border-t-2 border-b-2 border-black">
                <TableCell className="p-3 border-r border-gray-300 text-center">
                  Grand Total (in Rs)
                </TableCell>
                <TableCell className="p-3 border-r border-gray-300 text-right tabular-nums text-sm">
                  ₹ {gtPreceding.toLocaleString("en-IN", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                </TableCell>
                <TableCell className="p-3 border-r border-gray-300 text-right tabular-nums text-sm">
                  ₹ {gtCurrent.toLocaleString("en-IN", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                </TableCell>
                <TableCell className="p-3 border-r border-gray-300 text-right tabular-nums text-sm">
                  ₹ {gtNext.toLocaleString("en-IN", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                </TableCell>
                <TableCell colSpan={2}></TableCell>
              </TableRow>
            </TableBody>
