...
export default function SamabyathiStatusPage() {
  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    setStatusData(null);
    try {
      const res = await fetch(`/api/samabathy/status?applicationNumber=${encodeURIComponent(appNumber.trim())}`);
      ...
      <div className="max-w-2xl mx-auto space-y-6">
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Enter your Samabyathi application number to check the current status of your request.
            </CardDescription>
          </CardHeader>
          <CardContent>
          ...
          <Card className="animate-in fade-in slide-in-from-bottom-4 duration-500">
            <CardHeader className="flex flex-row items-center justify-between pb-2">
              <div className="space-y-1">
                ...
                <CardDescription>Application Number: <span className="font-mono font-bold text-primary">{statusData.applicationNumber}</span></CardDescription>
              </div>
            ...
            <CardContent className="space-y-4">
              <div className="grid grid-cols-2 gap-4 pt-4">
                ...
                <div>
                  ...
                  <p className="font-medium">{formatDate(statusData.createdAt)}</p>
                </div>
              </div>

              {statusData.status === "APPROVED" && statusData.sanctionAmount && (
                <div className="mt-6 p-4 bg-green-50 border border-green-100 rounded-lg">
                  <p className="text-sm text-green-700 font-medium">Sanctioned Amount</p>
                  <p className="text-2xl font-bold text-green-900">₹ {statusData.sanctionAmount}</p>
                  <p className="text-xs text-green-600 mt-1">Please contact your Gram Panchayat office for disbursement details.</p>
                </div>
              )}
              ...
              {statusData.status === "PENDING" && (
                <div className="mt-6 p-4 bg-yellow-50 border border-yellow-100 rounded-lg">
                  ...
                  <p className="text-sm text-yellow-600 mt-1">
                    Your application is currently being reviewed by the administration. 
                    Approval depends on verification and availability of funds.
                  </p>
                </div>
              )}
            </CardContent>
