...
import {
  ...
  Wallet, PlusCircle, CheckCircle2, AlertCircle, FileText, LayoutGrid, Activity
} from "lucide-react";
...
export default function AddActionPlanForm() {
  ...
  async function onSubmit(values: z.infer<typeof actionplanschema>) {
    ...
    setIsLoading(true);
    try {
      const res = await createschme(values);
      if ((res as any).error) {
        toast({ title: "Error", description: (res as any).error, variant: "destructive" });
      } else {
        toast({ title: "Success", description: "Action Plan created successfully!" });
  ...
  return (
    <div className="max-w-5xl mx-auto py-6">
      <div className="mb-8">
        ...
        <p className="text-gray-500 mt-1">Fill out the details below to add a new project to the action plan.</p>
      </div>
        ...
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
          ...
          <div className="flex justify-end gap-4 pt-4">
            ...
            <Button type="submit" disabled={isLoading || !isFundMatched || estimatedCost === 0} className="bg-blue-600 hover:bg-blue-700 text-white min-w-[150px]">
              {isLoading ? "Saving..." : "Create Action Plan"}
            </Button>
          </div>
        </form>
      </Form>
