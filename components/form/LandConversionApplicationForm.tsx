...
export default function LandConversionApplicationForm({
  ...
  const FormFooter = () => (
    <Card className="shadow-lg border-indigo-100/60 bg-white/70 backdrop-blur-xl overflow-hidden transition-all duration-300 mt-8">
      <CardContent className="pt-6 bg-gradient-to-r from-indigo-50/30 to-transparent">
        <div className="flex flex-col sm:flex-row gap-4 items-center justify-between">
          <div className="flex items-center gap-2 text-sm text-slate-600 bg-white/60 px-4 py-2 rounded-full border border-indigo-100/50 shadow-sm backdrop-blur-sm">
            <AlertCircle className="h-4 w-4 text-indigo-500" />
            <span className="font-medium">Review all mandatory fields (*) before submission</span>
          </div>
          ...
          <div className="flex gap-4 w-full sm:w-auto">
            <Button
              type="button"
              variant="outline"
              className="flex-1 sm:flex-none border-indigo-200 text-indigo-700 hover:bg-indigo-50 transition-all active:scale-95 shadow-sm rounded-xl h-12 px-6"
            ...
            <Button
              type="button"
              className="flex-1 sm:flex-none bg-indigo-600 hover:bg-indigo-700 shadow-md hover:shadow-lg transition-all active:scale-95 rounded-xl text-white h-12 px-8"
              disabled={isSubmitting || isPending}
              onClick={handleSubmit((values) =>
