...
import { MapPin, Zap, Calendar, Phone, Eye, MessageSquarePlus } from "lucide-react";
      ...
      <div className="min-w-[240px] max-w-[280px] space-y-2 text-sm">
        ...
        {light.lightImageUrl && (
          <div className="rounded-lg overflow-hidden border">
            <Image src={light.lightImageUrl} alt="Light" width={280} height={140} className="object-cover w-full h-28" />
          </div>
        )}
        <Button size="sm" className="w-full gap-1.5" onClick={() => router.push(`/admindashboard/street-lights/register/${light.id}`)}>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
        <DetailRow label="Mouza" value={`${light.mouza?.mouzaName}${light.mouza?.jlNo ? ` (JL: ${light.mouza.jlNo})` : ""}`} />
        ...
        <DetailRow label="Wattage" value={light.wattage ? `${light.wattage} W` : undefined} icon={<Zap className="w-3.5 h-3.5 text-yellow-500" />} />
        <DetailRow label="Pole Type" value={light.poleType?.replace(/_/g, " ")} />
        <DetailRow label="Ownership" value={light.ownership?.replace(/_/g, " ")} />
        <DetailRow label="Installation Year" value={light.installYear?.toString()} icon={<Calendar className="w-3.5 h-3.5 text-muted-foreground" />} />
