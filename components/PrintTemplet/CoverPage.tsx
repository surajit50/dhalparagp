...
export default function CoverPagePrint({
  ...
  const handleGeneratePDF = async () => {
    ...
    const getPhotoByStatus = (status: string) =>
      workCoverPageType.workPhotos?.find(
        (p) => p.status === status && p.isVerified,
      );
    ...
    const [onsetBase64, ongoingBase64, completeBase64] = await Promise.all([
      onsetPhoto
        ? getBase64FromUrl(onsetPhoto.imageUrl)
        : Promise.resolve(null),
      ongoingPhoto
        ? getBase64FromUrl(ongoingPhoto.imageUrl)
        : Promise.resolve(null),
      completePhoto
        ? getBase64FromUrl(completePhoto.imageUrl)
        : Promise.resolve(null),
    ]);
